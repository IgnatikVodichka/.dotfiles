...
function register(rootUri, languageService, getTextDocument, getConfiguration, ts) {
    return (uri, range) => __awaiter(this, void 0, void 0, function* () {
        const document = getTextDocument(uri);
        if (!document)
            return;
        const preferences = yield (0, getUserPreferences_1.getUserPreferences)(getConfiguration, document.uri, rootUri);
        const fileName = shared.getPathOfUri(document.uri);
        const start = document.offsetAt(range.start);
        const end = document.offsetAt(range.end);
        let inlayHints = [];
        try {
            inlayHints = 'provideInlayHints' in languageService ? languageService.provideInlayHints(fileName, { start, length: end - start }, preferences) : [];
        }
        catch (_a) { }

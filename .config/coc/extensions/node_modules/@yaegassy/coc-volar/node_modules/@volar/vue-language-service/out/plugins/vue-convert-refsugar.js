...
const refSugarRanges = require("../utils/refSugarRanges");
...
function default_1(options) {
    ...
    return {
        ...
        codeLens: {
            on(document) {
                return worker(document.uri, (vueFile) => __awaiter(this, void 0, void 0, function* () {
                    var _a, _b;
                    if (document.uri.endsWith('.html')) // petite-vue
                        return;
                    const isEnabled = (_b = yield ((_a = context.env.configurationHost) === null || _a === void 0 ? void 0 : _a.getConfiguration('volar.codeLens.scriptSetupTools'))) !== null && _b !== void 0 ? _b : true;
                    if (!isEnabled)
                        return;
                    const result = [];
...
function useRefSugar(context, document, vueSourceFile, commandContext, findReferences, findTypeDefinition) {
    return __awaiter(this, void 0, void 0, function* () {
        const ts = context.typescript.module;
        const sfc = vueSourceFile.sfc;
        if (!sfc.scriptSetup)
            return;
        if (!sfc.scriptSetupAst)
            return;
        commandContext.workDoneProgress.begin('Use Ref Sugar', 0, '', true);
        const edits = yield getUseRefSugarEdits(document, sfc.scriptSetup, sfc.scriptSetupAst);
        if (commandContext.token.isCancellationRequested)
            return;
        if (edits === null || edits === void 0 ? void 0 : edits.length) {
            yield commandContext.applyEdit({ changes: { [document.uri]: edits } });
        }
        commandContext.workDoneProgress.done();
        function getUseRefSugarEdits(document, _scriptSetup, _scriptSetupAst) {
            var _a, _b;
            return __awaiter(this, void 0, void 0, function* () {
                    ...
                    for (const binding of declaration.leftBindings) {
                        ...
                        references = references.filter(reference => {
                            ...
                            if (end < _scriptSetup.startTagEnd || start > _scriptSetup.startTagEnd + _scriptSetup.content.length)
                                return false;
                            if ((0, vue_autoinsert_dotvalue_1.isBlacklistNode)(ts, _scriptSetupAst, start - _scriptSetup.startTagEnd, true))
                                return false;
                            return true;
                        });
        ...
        function isRefType(typeDefs) {
            ...
            for (const typeDefine of typeDefs) {
                ...
                if (!snapshot)
                    continue;
                const defineDoc = vscode_languageserver_textdocument_1.TextDocument.create(uri, 'typescript', 0, snapshot.getText(0, snapshot.getLength()));
...
function unuseRefSugar(ts, document, vueSourceFile, context, doCodeActions, doCodeActionResolve, doRename, doValidation) {
    var _a;
    return __awaiter(this, void 0, void 0, function* () {
        const sfc = vueSourceFile.sfc;
        if (!sfc.scriptSetup)
        ...
        const edits = yield getUnRefSugarEdits(document, sfc.scriptSetup, sfc.scriptSetupAst);
        if (context.token.isCancellationRequested)
            return;
        if (edits === null || edits === void 0 ? void 0 : edits.length) {
            yield context.applyEdit({ changes: { [document.uri]: edits } });
            yield shared.sleep(200);
            const errors = (_a = yield doValidation(document.uri)) !== null && _a !== void 0 ? _a : [];
            const importEdits = yield (0, vue_convert_scriptsetup_1.getAddMissingImportsEdits)(document, doCodeActions, doCodeActionResolve);
            const removeInvalidValueEdits = getRemoveInvalidDotValueEdits(document, errors);
            if (importEdits && removeInvalidValueEdits) {
                (0, language_service_1.mergeWorkspaceEdits)(importEdits, removeInvalidValueEdits);
        ...
        function getRemoveInvalidDotValueEdits(document, errors) {
            const edits = [];
        ...
        function getUnRefSugarEdits(document, _scriptSetup, _scriptSetupAst) {
            return __awaiter(this, void 0, void 0, function* () {
                ...
                for (const callRange of ranges.refs) {
                    ...
                    for (const binding of callRange.leftBindings) {
                        ...
                        const renames = yield doRename(document.uri, document.positionAt(_scriptSetup.startTagEnd + binding.end), bindingName + '.value');
...
function getRanges(ts, scriptSetupAst) {
    return {
